...
# Known Problems:
#   Switching viewports while the client is loading sometimes allows the
#   artifact to show again.
...
# -- Constants -----------------------------------------------------------------
    ...
    readonly version="1.0.4.beta" # major.minor.point.stage
...
function checkJVM() {
    # quetly test if both java is running and that java is minecraft via 
    # grep exit code. this identifies the minecraft client and not the launcher.
    ps ax | grep java | grep minecraft | grep -v grep > /dev/null
    if [ "$?" = "0" ]; then
        jvmIsRunning=true
    fi
}
...
function main() {  
    ...
    # start FTB launcher
    java -jar $ftbFileName >> $workingDir/launcher.log
